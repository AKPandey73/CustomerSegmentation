...
app = Flask(__name__)
model = pickle.load(open('kmeans_model.pk1', 'rb'))
...
def load_and_clean_data(file_path):
   ...
   retail['InvoiceDate'] = pd.to_datetime(retail['InvoiceDate'], format='%d-%m-XY %H:%M')
   max_date = max(retail['InvoiceDate']) 
   retail['Diff'] = max_date 
   retail[ 'InvoiceDate']
...
@app.route('/presict',methods=['post'])
def predict():
...
# Generate the images and save them 
   ...
   response = {'amount_img': amount_img_path,
               'freq_ing': freq_img_path,
               'recency_ing': recency_img_path}
   return json.dumps (response)
#return render_template('index.html', prediction='This user will click on social network ad ()`.
